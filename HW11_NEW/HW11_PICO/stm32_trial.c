...
int main(void) {
	...
	while (1) {
		// Read from USB
		char uart_rx_message[100];
		char usb_rx_message[100];

		HAL_UART_Receive(&hcom_uart[COM1], (uint8_t*)&usb_rx_message, 100, 10);
		...
		for (int i = 0; i < 100; i++){
			if (usb_rx_message[i] == '\n'){
				...
				usb_buffer[usb_index] = usb_rx_message[i];
		...
		// Read from Pico
		HAL_UART_Receive(&huart1, (uint8_t*)&uart_rx_message, 100, 10);


		for (int i = 0; i < 100; i++){
			if (uart_rx_message[i] == '\n'){
				...
				uart_buffer[uart_index] = uart_rx_message[i];
