...
//	int uart_index = 0;
	...
	// Interrupt for receiving from pico
...
//				HAL_UART_Transmit(&hcom_uart[COM1], (uint8_t*)"\n", 1, 10);
		...
		// Read from Pico
		if (HAL_UART_Receive(&huart1, (uint8_t*)&uart_rx_char, 1, 100) == HAL_OK){
...
//				 print to STM32 virtual COM port
				HAL_UART_Transmit(&hcom_uart[COM1], (uint8_t*)uart_buffer, strlen(uart_buffer), 100);
				char new_line[] = "\n";

				HAL_UART_Transmit(&hcom_uart[COM1], (uint8_t*) new_line, 1, 10);
//				printf("Received: %s\n", uart_buffer);
...
//            uart_buffer[uart_index] = '\0';
//
//            // Don't use printf - transmit directly
//            HAL_UART_Transmit(&hcom_uart[COM1], (uint8_t*)"From Pico: ", 11, 100);
//            HAL_UART_Transmit(&hcom_uart[COM1], (uint8_t*)uart_buffer, uart_index, 100);
//            HAL_UART_Transmit(&hcom_uart[COM1], (uint8_t*)"\r\n", 2, 100);
//            uart_index = 0;
//        }
//        else
//        {
//            if (uart_index < 999) uart_buffer[uart_index++] = uart_rx_char;
//        }
